...
class CBCS(QThread):
    ...
    def __init__(self, parent_url, start_usn, end_usn, file_path=None, parent=None):
        super(CBCS, self).__init__(parent)
        self.parent_url = parent_url
        self.file_path = file_path
        self.start_usn = start_usn
        self.end_usn = end_usn
        self.usn_base = self.start_usn[:-3]
        self.child_url = 'http://results.vtu.ac.in/vitavirevalresultcbcs2018/resultpage.php'
    ...
    def run(self):
        ...
        for usn in range(int(self.start_usn[7:]), int(self.end_usn[7:]) + 1):
            ...
            result_data = dict(lns=usn_full, current_url=self.parent_url)
...
if __name__ == '__main__':
    ...
    test_obj = CBCS(url, '1RN15EC001', '1RN15EC30', 'CBCS_Result.xlsx')
    test_obj.run()
