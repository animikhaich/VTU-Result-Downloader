...
class NON_CBCS(QThread):
    non_cbcs_update_progress = pyqtSignal(float)

    def __init__(self, parent_url, start_usn, end_usn, file_path=None, parent=None):
        super(NON_CBCS, self).__init__(parent)
        self.parent_url = parent_url
        self.file_path = file_path
        self.start_usn = start_usn
        self.end_usn = end_usn
        self.usn_base = self.start_usn[:-3]
        self.child_url = 'http://results.vtu.ac.in/vitaviresultnoncbcs18/resultpage.php'
    ...
    def run(self):
        ...
        for usn in range(int(self.start_usn[7:]), int(self.end_usn[7:]) + 1):

            if len(str(usn)) < 3:
                usn_str = '0' * (3 - len(str(usn))) + str(usn)
            else:
                usn_str = str(usn)

            usn_full = self.usn_base + usn_str

            progress = (usn - int(self.start_usn[7:])) / (int(self.end_usn[7:]) - int(self.start_usn[7:])) * 100
            self.non_cbcs_update_progress.emit(progress)
            ...
            try:
                total_marks = soup.find("td", {"style": "padding-left:10px"}).text.strip(': ')
            ...
            all_subs_df = all_subs_df.rename_axis({0: total_marks}, axis='index')
            df = pd.concat([all_subs_df], keys={name: all_subs_df})
            df = pd.concat([df], keys={usn_full: df})
    ...
    def write_excel(self):
        if len(self.file_path) < 1:
            self.file_path = 'NON_CBCS_Result.xlsx'
...
if __name__ == '__main__':
    ...
    test_obj = NON_CBCS(url, '1RN14EC001', '1RN14EC100', 'NON_CBCS_Result.xlsx')
    test_obj.run()
